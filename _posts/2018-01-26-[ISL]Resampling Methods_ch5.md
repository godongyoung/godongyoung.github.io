...
2개의 회사에 돈을 투자할 수 있다고 가정해보자. 각각의 회사는 미지의 변수 X와 Y만큼의 투자이익을 줄 것이다. 현재 가진 돈 중 X회사에 $$\alpha$$만큼의 비율을투자하고 남은돈을 Y회사에 투자할때, 미지의 변수 X와 Y의 Variance를 고려해서, 전체 risk가 적은, 즉 $$Var(\alpha X+(1-\alpha)Y)$$를 최소화할 수 있는 비율 $$\alpha$$로 두 회사에 투자하려한다. 이 경우, 간단한 정리를 통해 해당 Var를 최소화하는 $$\alpha$$는 다음과 같다는 것을 도출할 수 있다.
