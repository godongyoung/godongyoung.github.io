...
이전에 말
$$Pr(Y=파랑|X=x_{0}) $$
(그림에서 맨 오른쪽, 혹은 맨 아래의 점들은 모두 파란색이므로 이때의 conditional probability  $$Pr(Y=파랑|X=x_{0})$$는 1이다.) 애초에 모집단의 분포 자체가 섞여 있을 수도 있다(그림에서 정 중앙라인 부분). 그럴 경우 100% 그 클래스에 속하는 것은 아니고, 해당 경우 $$Pr(Y=파랑|X=x_{0})$$는 1보다 작다. 

(그림에서 맨 오른쪽, 혹은 맨 아래의 점들은 모두 파란색이므로 이때의 conditional probability 

$$Pr(Y=파랑|X=x_{0})$$

는 1이다.) 애초에 모집단의 분포 자체가 섞여 있을 수도 있다(그림에서 정 중앙라인 부분). 그럴 경우 100% 그 클래스에 속하는 것은 아니고, 해당 경우 $$Pr(Y=파랑|X=x_{0})$$는 1보다 작다. 
